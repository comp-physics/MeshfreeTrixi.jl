...
"""
    PolyharmonicSplineBasis([RealT=Float64,] polydeg::Integer)
...
"""
struct PolyharmonicSplineBasis{RealT <: Real, NNODES,
...
function PolyharmonicSplineBasis(RealT, polydeg::Integer)
    nnodes_ = polydeg + 1

    # compute everything using `Float64` by default
    nodes_, weights_ = polyharmonic_nodes_weights(nnodes_)
    ...
    return PolyharmonicSplineBasis{RealT, nnodes_, typeof(nodes),
...
PolyharmonicSplineBasis(polydeg::Integer) = PolyharmonicSplineBasis(Float64, polydeg)

function Base.show(io::IO, basis::PolyharmonicSplineBasis)
    @nospecialize basis # reduce precompilation time

    print(io, "PolyharmonicSplineBasis{", real(basis), "}(polydeg=", polydeg(basis),
          ")")
end
function Base.show(io::IO, ::MIME"text/plain", basis::PolyharmonicSplineBasis)
    @nospecialize basis # reduce precompilation time

    print(io, "PolyharmonicSplineBasis{", real(basis), "} with polynomials of degree ",
          polydeg(basis))
end

function Base.:(==)(b1::PolyharmonicSplineBasis, b2::PolyharmonicSplineBasis)
...
@inline Base.real(basis::PolyharmonicSplineBasis{RealT}) where {RealT} = RealT

@inline function nnodes(basis::PolyharmonicSplineBasis{RealT, NNODES}) where {RealT,
                                                                              NNODES
                                                                              }
    NNODES
end
...
"""
    eachnode(basis::PolyharmonicSplineBasis)

Return an iterator over the indices that specify the location in relevant data structures
for the nodes in `basis`. 
In particular, not the nodes themselves are returned.
"""
@inline eachnode(basis::PolyharmonicSplineBasis) = Base.OneTo(nnodes(basis))

@inline polydeg(basis::PolyharmonicSplineBasis) = nnodes(basis) - 1

@inline get_nodes(basis::PolyharmonicSplineBasis) = basis.nodes

"""
    integrate(f, u, basis::PolyharmonicSplineBasis)

Map the function `f` to the coefficients `u` and integrate with respect to the
quadrature rule given by `basis`.
"""
function integrate(f, u, basis::PolyharmonicSplineBasis)
...
left_boundary_weight(basis::PolyharmonicSplineBasis) = first(basis.weights)
right_boundary_weight(basis::PolyharmonicSplineBasis) = last(basis.weights)
...
function MortarL2(basis::PolyharmonicSplineBasis)
    RealT = real(basis)
...
# function MortarEC(basis::PolyharmonicSplineBasis{RealT}, surface_flux)
...
struct PolyharmonicSplineAnalyzer{RealT <: Real, NNODES,
                                  VectorT <: AbstractVector{RealT},
...
function SolutionAnalyzer(basis::PolyharmonicSplineBasis;
    ...
    return PolyharmonicSplineAnalyzer{RealT, nnodes_, typeof(nodes), typeof(vandermonde)
                                      }(nodes,
                                        weights,
                                        vandermonde)
end

function Base.show(io::IO, analyzer::PolyharmonicSplineAnalyzer)
    @nospecialize analyzer # reduce precompilation time

    print(io, "PolyharmonicSplineAnalyzer{", real(analyzer), "}(polydeg=",
          polydeg(analyzer), ")")
end
function Base.show(io::IO, ::MIME"text/plain", analyzer::PolyharmonicSplineAnalyzer)
    @nospecialize analyzer # reduce precompilation time

    print(io, "PolyharmonicSplineAnalyzer{", real(analyzer),
          "} with polynomials of degree ", polydeg(analyzer))
end

@inline Base.real(analyzer::PolyharmonicSplineAnalyzer{RealT}) where {RealT} = RealT

@inline function nnodes(analyzer::PolyharmonicSplineAnalyzer{RealT, NNODES}) where {
                                                                                    RealT,
                                                                                    NNODES
                                                                                    }
    NNODES
...
"""
    eachnode(analyzer::PolyharmonicSplineAnalyzer)

Return an iterator over the indices that specify the location in relevant data structures
for the nodes in `analyzer`. 
In particular, not the nodes themselves are returned.
"""
@inline eachnode(analyzer::PolyharmonicSplineAnalyzer) = Base.OneTo(nnodes(analyzer))

@inline polydeg(analyzer::PolyharmonicSplineAnalyzer) = nnodes(analyzer) - 1
...
function AdaptorL2(basis::PolyharmonicSplineBasis{RealT}) where {RealT}
    nnodes_ = nnodes(basis)
...
function polyharmonic_nodes_weights(n_nodes::Integer)
    # From Kopriva's book
    n_iterations = 10
    tolerance = 1e-15
