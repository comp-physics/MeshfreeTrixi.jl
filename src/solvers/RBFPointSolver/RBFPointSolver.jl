...
"""
PointCloudSolver(; RealT=Float64, polydeg::Integer,
            surface_flux=flux_central,
            surface_integral=SurfaceIntegralWeakForm(surface_flux),
            volume_integral=VolumeIntegralWeakForm(),
            mortar=MortarL2(basis))

Create a point cloud based solver using a
[`PolyharmonicSplineBasis`](@ref) with polynomials of degree `polydeg`.
"""
const PointCloudSolver = MeshfreeSolver{Basis} where {Basis <: PolyharmonicSplineBasis}

# TODO: Deprecated in v0.3 (no longer documented)
function DGSEM(basis::PolyharmonicSplineBasis,
               surface_flux = flux_central,
               volume_integral = VolumeIntegralWeakForm(),
               mortar = MortarL2(basis))
    surface_integral = SurfaceIntegralWeakForm(surface_flux)
...
function DGSEM(RealT, polydeg::Integer,
               surface_flux = flux_central,
               volume_integral = VolumeIntegralWeakForm(),
               mortar = MortarL2(PolyharmonicSplineBasis(RealT, polydeg)))
    basis = PolyharmonicSplineBasis(RealT, polydeg)
