...
DomainData: contains info for a high order piecewise polynomial discretization on an
unstructured mesh. 
...
md = DomainData(VXY, EToV, rd)
(; x, y ) = md
```
"""
Base.@kwdef struct DomainData{Dim, MeshType,
...
# DomainData constructor where we do not specify `nxyz` and instead compute it from `nrstJ` and `Jf`
function DomainData(mesh_type, FToF, xyz, xyzf, xyzq, wJq, mapM, mapP, mapB, rstxyzJ, J,
                    nxyzJ, Jf, is_periodic)
    nxyz = map(nJ -> nJ ./ Jf, nxyzJ)

    return DomainData(mesh_type, FToF, xyz, xyzf, xyzq, wJq, mapM, mapP, mapB, rstxyzJ, J,
                      nxyz, nxyzJ, Jf, is_periodic)
end

function ConstructionBase.setproperties(md::DomainData, patch::NamedTuple)
    fields = (haskey(patch, symbol) ? getproperty(patch, symbol) : getproperty(md, symbol) for symbol in fieldnames(typeof(md)))
    return DomainData(fields...)
end

function ConstructionBase.getproperties(md::DomainData)
...
function Base.show(io::IO, md::DomainData{DIM}) where {DIM}
    @nospecialize md
    print(io, "DomainData{$DIM}")
end

function Base.show(io::IO, ::MIME"text/plain",
                   md::DomainData{DIM, MeshType}) where {DIM, MeshType}
    @nospecialize md
    print(io,
          "DomainData of dimension $DIM with $(md.num_elements) elements with periodicity = $(md.is_periodic).")
end

function Base.propertynames(x::DomainData{1}, private::Bool = false)
    return (fieldnames(DomainData)...,
            :num_elements, :VX, :x, :xq, :xf, :nx, :rxJ)
end
function Base.propertynames(x::DomainData{2}, private::Bool = false)
    return (fieldnames(DomainData)...,
            :num_elements, :VX, :VY, :x, :y, :xq, :yq, :xf, :yf,
            :nx, :ny, :rxJ, :sxJ, :ryJ, :syJ)
end
function Base.propertynames(x::DomainData{3}, private::Bool = false)
    return (fieldnames(DomainData)...,
...
@inline function meshdata_getproperty(x::DomainData, s::Symbol)
    if s == :x
...
# generic fallback
Base.getproperty(x::DomainData, s::Symbol) = meshdata_getproperty(x, s)

"""
    struct VertexMappedMesh

The default `DomainData` mesh type, represents a mesh which is defined purely by 
...
function Base.getproperty(x::DomainData{Dim, <:VertexMappedMesh}, s::Symbol) where {Dim}
    if s === :VX
...
@inline num_elements(md::DomainData) = size(first(getfield(md, :xyz)), 2) # number of columns in the coordinate array

# splat `uniform_mesh` arguments, e.g., enables `DomainData(uniform_mesh(Line(), 1), rd)`
# TODO: wrap `uniform_mesh` in a custom type so we can dispatch more precisely?
"""
    DomainData(VXYZ, EToV, rd::RefElemData)
    DomainData((VXYZ, EToV), rd::RefElemData)

Returns a DomainData struct with high order DG mesh information from the unstructured
mesh information (VXYZ..., EToV).

    DomainData(rd::RefElemData, md::DomainData, xyz...)

Given new nodal positions `xyz...` (e.g., from mesh curving), recomputes geometric terms
and outputs a new DomainData struct. Only fields modified are the coordinate-dependent terms
    `xyz`, `xyzf`, `xyzq`, `rstxyzJ`, `J`, `nxyzJ`, `Jf`.
"""
DomainData(mesh::Tuple{<:Tuple, Matrix{Int64}}, rd::RefElemData, other_args...; kwargs...) = DomainData(mesh...,
                                                                                                        rd,
                                                                                                        other_args...;
                                                                                                        kwargs...)

function DomainData(VXYZ, EToV, rd::RefElemData, other_args...; kwargs...)
    DomainData(VXYZ..., EToV, rd, other_args...; kwargs...)
end # splats VXYZ 

function DomainData(VX::AbstractVector, EToV, rd::RefElemData{1}; is_periodic = (false,))
    ...
    periodicity = (false,)
    md = DomainData(VertexMappedMesh(rd.element_type, tuple(VX), EToV), FToF,
...
function DomainData(VX, VY, EToV, rd::RefElemData{2}; is_periodic = (false, false))
    (; fv) = rd
    ...
    md = DomainData(VertexMappedMesh(rd.element_type, tuple(VX, VY), EToV), FToF,
...
function DomainData(VX, VY, VZ, EToV, rd::RefElemData{3};
    ...
    md = DomainData(VertexMappedMesh(rd.element_type, tuple(VX, VY, VZ), EToV), FToF,
...
function DomainData(rd::RefElemData, md::DomainData{Dim}, xyz...) where {Dim}
